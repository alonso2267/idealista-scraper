document.addEventListener('DOMContentLoaded', () => {
  const API_KEY = 'bef8f0d5d713d42f1db35955744829fe';
  ...
  function cargarDatos(urlIdealista) {
    ...
    document.getElementById('propiedades').innerHTML = `<tr><td colspan="5" class="text-center">Cargando...</td></tr>`;
    ...
    fetch(URL)
      ...
      .catch(error => {
        ...
        document.getElementById('propiedades').innerHTML = `<tr><td colspan="5" class="text-center text-danger">Error al cargar los datos</td></tr>`;
      });
  }
  ...
  function aplicarFiltros() {
    ...
    const filtradas = propiedadesGuardadas.filter(p => {
      const condHabs = habs ? (habs === '4' ? p.habitaciones >= 4 : p.habitaciones == habs) : true;
      const condPrecio = p.precio >= precioMin && p.precio <= precioMax;
      const condMetros = p.metros >= metrosMin;
  ...
  function mostrarTabla(props) {
    ...
    if (props.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" class="text-center">No hay propiedades que coincidan con los filtros.</td></tr>`;
      return;
    }
